...
def identificar_enfermedad():
    ...
    if enfermedades_identificadas:
        ...
        resultado_texto.delete(1.0, tk.END)  # Limpiar resultados anteriores
...
def abrir_ventana_agregar_enfermedad():
    ...
    # Etiquetas y entradas para agregar nueva enfermedad
    tk.Label(ventana_agregar, text="Nueva Enfermedad:").grid(row=0, column=0, sticky=tk.W)
    entrada_enfermedad = tk.Entry(ventana_agregar)
    entrada_enfermedad.grid(row=0, column=1, sticky=tk.W)

    tk.Label(ventana_agregar, text="Síntomas de la Nueva Enfermedad (separados por coma y espacio):").grid(row=1, column=0, sticky=tk.W)
    entrada_sintomas_nuevos = tk.Entry(ventana_agregar, width=50)
    entrada_sintomas_nuevos.grid(row=1, column=1)

    tk.Label(ventana_agregar, text="Tratamiento de la Nueva Enfermedad:").grid(row=2, column=0, sticky=tk.W)
    entrada_tratamiento = tk.Entry(ventana_agregar, width=50)
    entrada_tratamiento.grid(row=2, column=1)

    # Botón para agregar nueva enfermedad
    tk.Button(ventana_agregar, text="Agregar Nueva Enfermedad", command=lambda: agregar_enfermedad(entrada_enfermedad.get(), entrada_sintomas_nuevos.get(), entrada_tratamiento.get())).grid(row=3, columnspan=2)
...
# Etiqueta y entrada para los síntomas
tk.Label(ventana, text="Síntomas (separados por coma y espacio):").grid(row=0, column=0, sticky=tk.W)
entrada_sintomas = tk.Entry(ventana, width=50)
entrada_sintomas.grid(row=0, column=1)

# Botón para identificar enfermedad y mostrar tratamiento
tk.Button(ventana, text="Identificar Enfermedad y Mostrar Tratamiento", command=identificar_enfermedad).grid(row=1, columnspan=2)

# Widget de texto para mostrar los resultados
resultado_texto = tk.Text(ventana, height=10, width=50)
resultado_texto.grid(row=2, columnspan=2)

# Botón para abrir ventana de agregar nueva enfermedad
tk.Button(ventana, text="Agregar Nueva Enfermedad", command=abrir_ventana_agregar_enfermedad).grid(row=3, columnspan=2)
